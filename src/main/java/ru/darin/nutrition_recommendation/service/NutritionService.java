...
// -
// - попробовать добавить запрос на подтверждение удаления: точно хотите удалить!?
// возможно нужно будет добавить поле 'description' для продуктов + добавить фото, а также блюда на основе этого продукта
// добавить к заболеваниям поле 'симптомы'
// попробовать при составлении микса для болезни убирать из списка уже добавленные продукты
// а также для типов продуктов
// Описать ошибку при сохранении Mix через сеттеры - не сохраняет в БД
// сохранение идет через Конструктор
// ------ узнать, почему один и тот же продукт в разных категориях ------
// сделать страницу для выбора двух заболеваний и списка продуктов для них (РАЗРЕШЕНО, ЗАПРЕЩЕНО)
